 ...
 * 
 * code added by: Yukun Shan (yshan@bu.edu)
 */
...
public class LetterSquare {
    ...
    /*
     * 
     * Last - returns the string that is formed by removing
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        ...
        if (letter == null || letter.length() != 1) throw new IllegalArgumentException();

        if (charNum == 0 && wordNum != 0){
            if (lastLetter(words[wordNum-1]).equals(letter)){
                return true;
            }else{
                return false;
            }
        ...
        if (!this.onSameSide(lastLetter(this.words[wordNum]), letter) && 
            dictionary.hasString(words[wordNum] + letter) && !alreadyUsed(words[wordNum] + letter)){
            return true;
        }
        return false;
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        ...
        if (this.allLettersUsed() && this.dictionary.hasFullWord(words[wordNum])){
                this.printSolution(wordNum);
                return true;
        } // base case one
        if (wordNum >= maxWords) return false; //base case two
        
        for (int i = 0; i < letters.length; i++){            
            if (this.isValid(letters[i], wordNum, charNum)){
                ...
                if (dictionary.hasFullWord(words[wordNum]) 
                    && words[wordNum].length() >= 3){
